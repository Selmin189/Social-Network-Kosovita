...
public class EditPostActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String initialPostContent = intent.getStringExtra("post");
        String initialPostImage = intent.getStringExtra("statusImage");
        privacylevel = intent.getIntExtra("privacy", 0);
        Log.d("EditPostActivity", "Initial post ID: " + postId);


        textInputEditText.setText(initialPostContent);

        if (initialPostImage != null && !initialPostImage.isEmpty()) {
            Glide.with(this)
                    .load(initialPostImage)
        ...
        postTV.setOnClickListener(v -> {
            ...
            if (status.trim().length() > 0 || isImageSelected) {
                ...
                postUploadViewModel.editPost(multipartBody).observe(EditPostActivity.this, new Observer<GeneralResponse>() {
                    @Override
                    public void onChanged(GeneralResponse generalResponse) {
                        progressDialog.hide();
                        Toast.makeText(EditPostActivity.this, generalResponse.getMessage(), Toast.LENGTH_SHORT).show();
                ...
                Toast.makeText(EditPostActivity.this, "Please write your status", Toast.LENGTH_SHORT).show();
            }
        });
    }
